...
export function parseTimelineJson(data: unknown): GeoPoint[] {
  ...
  if (Array.isArray(data)) {
    ...
    throw new TimelineParseError(
      'legacy-format',
      'Bu eski bir Google Takeout biçimi. Bunun yerine telefonundan zaman çizelgesi verisini dışa aktar.',
    );
  } else if (isObject(data) && 'rawSignals' in data) {
    throw new TimelineParseError(
      'raw-signals-only',
      'Bu dosyada ham sinyaller var ama işlenmiş yolculuk yok.',
    );
  } else {
    throw new TimelineParseError(
      'unsupported-format',
  ...
  if (normalized.length === 0) {
    throw new TimelineParseError(
      'no-usable-locations',
      'Bu dosyada kullanılabilir konum noktası yok.',
    );
  }
  return normalized;
}

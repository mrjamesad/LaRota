...
const CAMERA_TRACK_SAMPLES = 1440;
...
export function worldPositionAtProgress(journey: CameraJourney, progress: number): WorldPosition {
  const distanceKm = journey.pacing
    ? distanceAtProgress(journey.pacing, progress)
    : journey.totalDistanceKm * clamp(progress, 0, 1);
  return worldPositionAtDistance(journey, distanceKm);
}
...
export function overviewSafeArea(width: number, height = width): OverviewSafeArea {
...
export function overviewViewport(journey: CameraJourney, width: number, height = width): Viewport {
  ...
  const spanX = Math.max(worldPerPixel * width, MIN_OVERVIEW_VIEWPORT_SPAN);
  const spanY = clamp(
